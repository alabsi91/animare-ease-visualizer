...
export default function SmallSidePanel() {
  ...
  const togglePathPoints: React.MouseEventHandler<HTMLButtonElement> = e => {
    ...
    const autoHideElements = document.querySelectorAll<HTMLAnchorElement>('.auto-hide');
    for (const item of autoHideElements) {
      if (item) item.style.display = ctx.autoHideHandles ? 'none' : 'block';
    }
  };
  ...
  const open = () => {
    ...
    const sidePanel = document.querySelector<HTMLDivElement>('.sidePanel');
    if (!sidePanel) return;
  ...
  const PresetsButton = useCallback(({ onClick }: { onClick: () => void }) => {
    return (
      <button type='button' className='small-side-panel-buttons' style={{ marginBottom: 0 }} title='presets' onClick={onClick}>
  ...
  const ExportButton = useCallback(({ onClick }: { onClick: () => void }) => {
    return (
      <button type='button' className='small-side-panel-buttons' style={{ marginBottom: 0 }} title='export' onClick={onClick}>
        <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
          <title>export</title>
  ...
  return (
    <div>
      <div className='small-side-panel custom-scrollbar'>
        <button type='button' title='show the side panel' className='open-panel' onClick={open}>
          <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
            <title>Open Large Side Panel</title>
            <polygon points='6.23,20.23 8,22 18,12 8,2 6.23,3.77 14.46,12' />
          </svg>
        </button>

        <button type='button' title='play current easing' onClick={ctx.playCurrentEasing}>
          <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
            <title>Play Current Easing</title>
        ...
        <button type='button' title='Enable snapping to the grid' onClick={toggleMagnet}>
          <svg
            style={{ fill: !ctx.magnet.current ? 'var(--text-color)' : 'var(--main-color)' }}
            xmlns='http://www.w3.org/2000/svg'
            viewBox='0 0 24 24'
          >
            <title>Enable snapping to the grid</title>
        ...
        <button type='button' title='Auto hide anchor points.' onClick={togglePathPoints}>
          <svg
            style={{ fill: !ctx.autoHideHandles ? 'var(--text-color)' : 'var(--main-color)' }}
            xmlns='http://www.w3.org/2000/svg'
            viewBox='0 0 24 24'
          >
            <title>Auto hide anchor points.</title>
